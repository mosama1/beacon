	...
	<div class="principal">
		<ul class="links">
			...
			@if( $ultimo_paso == 2 and $actual == 'home')
				<li class="{{ ( $ultimo_paso >= 2 ) ? '' : 'desactivado' }}"  data-step="4" data-intro="Puedes crear los Servicios">
			@else
				<li class="{{ ( $ultimo_paso >= 2 ) ? '' : 'desactivado' }}">
			@endif
				<a href="{{ ( $ultimo_paso >= 2 ) ? route('all_type_plate') : '' }}">
					<img src="img/icons/servicio.png" title="Servicios">
				</a>
				<ul>
					<li class="{{ ( $ultimo_paso >= 2 ) ? '' : 'desactivado' }}">
						<a href="{{ ( $ultimo_paso >= 2 ) ? route('all_type_plate') : '' }}">
							<span>Tipos de Plato</span>
						</a>
					</li>
					<li class="{{ ( $ultimo_paso >= 2 ) ? '' : 'desactivado' }}">
